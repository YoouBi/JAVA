...
class Classification {
	...
	public void main() {
		...
		while(true) {
			...
			if(i > 6) System.out.println("없는 번호입니다.");
...
//		bookCopy[bookCopy.length - i] = new BookMark(null, null, null, null, 0);

		bookShelf = addBook(i);
	}
	
	private BookMark[] addBook(int i) {
		BookMark[] bookCopy = new BookMark[bookShelf.length + i];
		bookCopy = Arrays.copyOf(bookShelf, bookShelf.length);
