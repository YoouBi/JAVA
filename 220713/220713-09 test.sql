...
SELECT FIRST_NAME, LAST_NAME, SALARY, ((SELECT AVG(SALARY) FROM employees) - SALARY) AS SALARYGAP FROM employees;
...
SELECT * FROM employees WHERE SALARY > (
	SELECT SALARY FROM employees WHERE JOB_ID LIKE 'IT%' ORDER BY SALARY DESC LIMIT 1
) ORDER BY SALARY ASC;
